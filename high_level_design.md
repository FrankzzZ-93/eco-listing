...
将当前跨越 6 个工具的 12 步手动操作，重构为一套 **多 Agent 协作系统**。由 Orchestrator Agent 统一调度 5 个专业 Agent，每个 Agent 拥有独立的工具集、推理能力和自我纠错机制。用户仅需输入竞品 ASIN 列表和关键词词库（本品是待生成对象，无需 ASIN），系统通过 Browser Tool 自动采集竞品前台数据（五点描述、Alex 问答、评论），Agent 自主完成产品建模、关键词策略、文案撰写、合规审查和 ST 优化，在关键节点请求人工确认。
...
| **Browser Tool** | Playwright 驱动，自动采集 Amazon 商品页数据（Listing/Alex/评论） | Research Agent |
...
```
IF memory 缺少竞品数据:
    → 启动 Research Agent（自动采集竞品 Listing / Alex / 评论）
...
**与旧 Pipeline 的本质区别**：不是按固定的 Step 1 → 12 推进，而是根据当前"缺什么"来决定"做什么"。用户只需提供竞品 ASIN + 词库，Research Agent 自动采集竞品数据。如果用户直接上传了属性表，Orchestrator 会跳过 Research + Product Analyst，直接进入关键词和文案环节。

---

### 4.2 Research Agent（竞品调研 Agent）

**目标**：根据竞品 ASIN 列表，自动采集竞品前台数据（Listing 文本、Alex 问答、评论），写入 Shared Memory。

**可用工具**：Browser Tool, LLM Tool（多模态，用于解析 Alex 截图）, File Store Tool
...
2. 采集 Alex 问答
   - 在商品页定位 Alex AI 助手区域
   - 使用 Browser Tool 截图 Alex 问答区域
   - 使用 LLM Tool（多模态）从截图提取问题列表
   - 输出：alex_questions[]（如 "Can it hold heavy belts?"）
...
4. 校验采集完整性：
   ...
   - 如 Alex 区域不可见（部分品类/站点无 Alex）→ 跳过，标记为空
...
```
1. 从 Memory 读取：competitor_listings, review_summary, alex_questions, 截图
...
Round 2: Alex 优化
  输入：draft_v1 + alex_questions + approved_product_attributes
  模型：Claude Sonnet（精细优化）
  策略：确保 Listing 能回答 Alex 提出的消费者问题
  输出：draft_v2
...
| Alex 问题覆盖 | 检查每个 alex_question 是否在 listing 中有对应回答 |
...
Memory
  ...
  │     ├── alex_questions[]               (Research Agent 自动采集 + LLM 提取)
  │     ├── alex_screenshots[]             (文件路径，Browser Tool 截图)
  │     ├── product_attributes_draft{}
  │     ├── approved_product_attributes{}
...
[
  ...
  { "agent": "copywriter", "name": "round_2_alex_v1", "filename": "round_2_alex_v1.md", "modified": true }
]
```
...
```json
{ "agent": "copywriter", "name": "round_2_alex_v1", "content": "# Round 2...", "modified": true }
```
...
| 选择 | 理由 |
|------|------|
| Research Agent 合并 Step 1/2/3 | 三者都是"采集竞品信息"（Listing 文本 / Alex 问答 / 评论），由同一个 Agent 通过 Browser Tool 统一采集 |
...
| Agent | MVP 实现 | 自动化程度 |
|-------|---------|----------|
| Research Agent | Browser Tool 自动采集竞品 Listing + Alex + 评论 | **全自动**（失败时降级为手动上传） |
...
**MVP 用户输入**：
1. 竞品 ASIN 列表（2-4 个；本品是待生成对象，无需 ASIN）
2. 关键词词库（欧鹭等工具导出文件）
