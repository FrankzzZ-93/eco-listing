...
export default function ImageEditorPage() {
  ...
  const latestTask = useMemo(
    () => findLatestImageEditorTask(tasks, draft.latestTaskId),
    [draft.latestTaskId, tasks],
  )
  ...
  // eco_listing: editing runs through the codex backend, which needs no API key
  // and offers no engine choice, so upstream's profile/engine gating is dropped.
  ...
  const finishPointerAction = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    const start = points[0]
    if (currentAnnotation.kind === 'brush') {
      // Freehand strokes are judged by the path actually drawn, not by how far
      // the end landed from the start: a circle, a loop or any scribble that
      // returns near its origin has ~0 displacement and used to be thrown away
      // on release. A tap with no movement is kept as a dot — normalization
      // drops anything with fewer than 2 points, so it gets a second one.
      const committed = points.length >= 2
        ? currentAnnotation
        : { ...currentAnnotation, points: [start, { x: start.x + 0.0005, y: start.y }] }
      commitAnnotations([...draft.annotations, committed])
    } else {
      // Shapes are defined by their two corners, so a degenerate (zero-size)
      // rectangle / ellipse / arrow is still discarded.
      const end = points[points.length - 1]
  ...
  const generate = async () => {
    ...
    try {
      ...
      const taskId = await submitTaskWithInput({
        ...
        inputImages,
        params: createImageEditorParams(draft.resolution, dimensions),
      ...
      <div className="ios-material relative mb-4 flex flex-col gap-4 overflow-hidden p-4 sm:p-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="relative min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-emerald-50/90 px-2.5 py-1 text-[11px] font-semibold text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-200">
              codex
            </span>
          </div>
          <p className="mt-1.5 max-w-2xl text-sm leading-6 text-gray-500 dark:text-gray-400">用标注说明修改位置，再完成局部增删、颜色材质调整、草图渲染或多图融合。</p>
        </div>
        {/* eco_listing: upstream links to the Seedream product page / Volcengine
            prompt guide and offers an "编辑配置" shortcut to the API settings.
            None apply here — editing runs on codex and needs no configuration. */}
      </div>

      {/* eco_listing: upstream lets you switch between the home API profile and a
          dedicated Seedream Pro profile here. This fork has a single codex
          backend, so there is nothing to choose and the selector is dropped. */}
