// eco_listing: ported from upstream 868f9e5. Upstream drives this editor with
// Seedream 5.0 Pro over the Volcengine API (needs an API key); here the very same
// annotate-then-describe mechanism is fed to the codex backend instead, so all
// engine/profile switching is dropped — there is only one backend.
import type { SeedreamAnnotation, SeedreamEditorResolution, TaskParams, TaskRecord } from '../types'
...
/**
 * 恢复最近一次图片编辑任务。
 * 优先保留草稿显式指向的任务；草稿缺少引用时回退到已持久化的编辑历史。
 */
