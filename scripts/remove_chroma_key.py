...
Vendored into this repo (from codex's bundled ``.system/imagegen`` skill) so the
white-background generation step does NOT depend on a particular codex install
having the skill present, and so it can be invoked with an OS-correct absolute
path + the backend's own Python interpreter (which has Pillow) instead of a
bash-only ``${CODEX_HOME:-$HOME/.codex}`` expansion that breaks on Windows.
See ``app/tools/image_gen_tool.py``.
"""
